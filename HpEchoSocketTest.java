...
class HpEchoSocketTest {
    ...
    static String hostGame( player HostPlayer ) {
        String battleResults = "Victory!";
        System.out.println("Your SocketNumber is " + 59090);
        ServerSocket Server;
        Socket client;
        try {
        ...
        try (PrintWriter fireComand =
            ...
            while (HostPlayer.crew > 0 && !lastShot.equals("-1,-1")) {
                System.out.println("reciveing fire");
                lastShot= fireRecived.readLine();
                System.out.println(lastShot);
                int x = Integer.parseInt(lastShot.substring(0, 1));
                int y = Integer.parseInt(lastShot.substring(2));
